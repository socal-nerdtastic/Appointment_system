...
class Login():
    def __init__(self,master):
        ...
        self.master = tk.Frame(master, width=500, height=500,bg='#ffffff')
        self.master.pack()
        # administrator text
        self.heading = tk.Label(self.master,text="ADMINISTRATOR",font=('arial 35 bold'),fg='black',bg='#ffffff')
        self.heading.place(x=55,y=0)
        # sign in to get access text
        self.access = tk.Label(self.master,text="Please Sign In to get access",font=('arial 20'),fg='black',bg='#ffffff')
        self.access.place(x=80,y=50)
        # UserId label
        self.user_ID = tk.Label(self.master,text="User ID: ",font=('arial 14'),fg="black",bg="#ffffff")
        self.user_ID.place(x=40,y=223)
        # Password label
        self.password = tk.Label(self.master,text="Password: ",font=('arial 14'),fg="black",bg="#ffffff")
        self.password.place(x=40,y=263)
        # UserId entry box
        self.user_ID_ent = tk.Entry(self.master,width=40)
        self.user_ID_ent.place(x=150,y=228)
        # Password entry box
        self.password_ent = tk.Entry(self.master,width=40)
        self.password_ent.place(x=150,y=268)
        # gives user the user id and password
        # userid answer
        self.user_ans = tk.Label(self.master,text="UserID: admin",bg='white')
        self.user_ans.place(x=40,y=423)
        # password answer
        self.pass_ans = tk.Label(self.master,text="Password: admin",bg='white')
        self.pass_ans.place(x=40,y=453)
        # login button *** no cmd yet! ***
        self.login = tk.Button(self.master, text="Login", width=33, bg='#b3b3ff', command=self.login_validation)
